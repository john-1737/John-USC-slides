...
\begin{frame}[t]
    ...
    Previously we covered measuring total returns (HPR, EAR, etc).  Today we will talk about evaluating those returns.   The first step is to compare the returns with something.
...
\begin{frame}[t]
    ...
    One convention is to use the return on Trasury Bills (T-Bills) as the risk-free rate, because T-bills returns have very low volatiliy.\\
    \vspace{1em}
